            ...
            if(this.inBasinTC || simSettings.trackMode===1){
                if(newestSegment){
                    if(this.record.length>1 && (selectedStorm===this || selectedStorm===undefined)){
                        ...
                        if(simSettings.trackMode===1 || (t>=this.formationTime && (!this.dissipationTime || t<this.dissipationTime))) tracks.line(2,pos.x,pos.y,nextPos.x,nextPos.y);                          
                        if(simSettings.trackMode===1 || (t>=this.formationTime && (!this.dissipationTime || t<this.dissipationTime))) tracks.circle(5,pos.x,pos.y,nextPos.x,nextPos.y);                        
                    }
                    ...
                    for(let n=0;n<this.record.length-1;n++){
                        ...
                        tracks.line(2,pos.x,pos.y,nextPos.x,nextPos.y);                           
                        tracks.circle(5,pos.x,pos.y,nextPos.x,nextPos.y);                        
                    }
                }
            }
